Function Get-ECKNewModuleVersion
    {
        ...
        # Version 1.8 - 25/06/2022 - bug fix, Modules were installed twice, $LogPath was not used
        ...
        If (![String]::IsNullOrWhiteSpace($lastEval))
            {
                If ((Get-date -Date $LastEval) -eq ((get-date).date))
                    {
                        Write-ECKlog -Message "[Warning] Module $ModuleName, was already downloaded today, to save bandwidth, now new download will occurs until tomorrow !" -type 2 -Path $LogPath
        ...
        Catch
            {
                If (-not ($null -eq $version) -and $version -ne "0.0.0.0")
                    {Write-ECKlog -Message "[Warning] No internet connection available, continuing with local version $version of $ModuleName" -type 2 -Path $LogPath}
                Else
                    {Write-ECKlog -Message "[ERROR] No internet connection available, unable to load module $ModuleName !!!" -type 3 -Path $LogPath ; Exit 1}
            }
        ...
        if ([version]"$a" -ge [version]"$b")
            {
                If ((Get-Module -Name $ModuleName).version -lt [version]"$a")
                    ...
                    {Write-ECKlog -Message "Module $ModuleName Local version [$a] is equal or greater than online version [$b], no update requiered" -Path $LogPath ; $Iret = $False}
        ...
        else
            {
                If ($b -ne "0.0")
                    {
                        Write-ECKlog -Message "Module $ModuleName Local version [$a] is lower than online version [$b], Updating Module !" -Path $LogPath
                        If ($a -ne "0.0")
                            {
                                        ...
                                        If ($ModuleName -like "*endpointcloudkit*")
                                            {
                                                ...
                                                Write-ECKlog -Message "Overwriting another module to allow import of $ModuleName !" -Path $LogPath
                                            }
                                        Else
                                            {Write-ECKlog -Message  "Commandlet of Module $ModuleName already loaded using... ...another module, skipping import !" -Path $LogPath}
                                    }
                                else
                                    {
                                        If ($ModuleName -like "*Endpointcloudkit*")
                                            {Write-ECKlog -Message "[FATAL ERROR] unable to import endpointcloudkit, $($_.Exception[0]), Aborting !" -Type 3 -Path $LogPath ; Exit 1}
                                        else
                                            {Write-ECKlog -Message "[ERROR] unable to load Module $ModuleName, skipping import !" -type 3 -Path $LogPath}
                                    }
                            }
                ...
                Else
                    {
                        Write-ECKlog -Message "[ERROR] Module $ModuleName not found online, unable to download, aborting!" -type 3 -Path $LogPath
